<template>
  <div>
    <el-button type="info" @click="init" icon="el-icon-search" style="position: fixed; right: 20px; top: 10px">歇后语搜索</el-button>
        ...
        <el-table-column type="expand">
          <template slot-scope="props">
            <el-form label-position="left" inline class="demo-table-expand">
              <el-form-item label="歇后语:">
                <span>{{ props.row.riddle }}</span>
              </el-form-item>
              ...
              <el-form-item label="更多了解:">
                <a target="_blank" :href="'https://baike.baidu.com/item/'+ props.row.riddle" style="text-decoration: none; color: #409EFF">百度百科</a>
              </el-form-item>
            </el-form>
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    handleSizeChange (val) {
      ...
      this.pageSize = val
      this.$store.commit('changeAllegoricalPageSize', val)
      const temp = this.$store.state.allegoricalParams.temp
      const page = this.$store.state.allegoricalParams.page
      this.searchWords(temp, page, val)
      this.$refs.dv.scrollTop = 0
    },
    handleCurrentChange (val) {
      console.log(`当前页: ${val}`)
      this.$store.commit('changeAllegoricalPage', val)
      const temp = this.$store.state.allegoricalParams.temp
      const pageSize = this.$store.state.allegoricalParams.pageSize
      this.searchWords(temp, val, pageSize)
      this.$refs.dv.scrollTop = 0
    },
    searchWords (temp, page, pageSize) {
      this.$http.get(`allegorical?temp=${temp}&page=${page}&pageSize=${pageSize}`).then(res => {
    ...
    search () {
      ...
      if (!/^[\u4e00-\u9fa5]+$/.test(temp)) {
        ...
        this.$store.commit('updateAllegoricalParams', { temp: temp, page: this.page, pageSize: this.pageSize })
        loading.close()
      }
    }
  }
